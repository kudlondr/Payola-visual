        ...
        entity match {
            ...
            case e: cz.payola.common.entities.EmbeddingDescription =>
                Some(new EmbeddingDescription(e.owner.map(User(_)), e.uriHash, e.defaultVisualPlugin,
                    e.defaultCustomization, e.analysisResultId, new java.sql.Timestamp(e.lastUpdate.getTime())))
            case _ => None
        }
    }
}

class EmbeddingDescription (o: Option[User], uriH: String, defVisPlugin: Option[String], defCust: Option[String],
    aResulId: String, lUpdate: java.sql.Timestamp)
    (implicit val context: SquerylDataContextComponent)
    extends cz.payola.domain.entities.EmbeddingDescription(o, uriH, defVisPlugin, defCust, aResulId, lUpdate)
